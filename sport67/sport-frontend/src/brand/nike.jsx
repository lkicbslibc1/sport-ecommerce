...
export default function Nike({ onViewChange, user, setUser, cart, addToCart }) {
  ...
  // Filter products by Nike brand only - now using products from context
  const brandProducts = useMemo(() => {
    return products.filter(p => p.brand === 'Nike');
  }, [products]);
  ...
  }, [brandProducts, priceRange, selectedSizes, selectedColors]);

  const activeFilterCount =
    (priceRange < 5000 ? 1 : 0) + selectedSizes.length + selectedColors.length;
    ...
    >
      ...
      <main className="max-w-[1440px] mx-auto px-6 lg:px-12 py-16 pt-28">
        <nav className="py-8 flex items-center gap-2 font-label-sm text-on-surface-variant uppercase tracking-widest">
          ...
          <span className="material-symbols-outlined text-[14px]">chevron_right</span>
          <span className="text-primary font-black">Nike</span>
        </nav>

        <div className="mb-10">
          <h1 className="font-display-lg text-7xl md:text-8xl font-black italic tracking-tighter uppercase text-on-background">Nike</h1>
          <p className="text-on-surface-variant max-w-xl mt-4 font-body-lg font-light leading-relaxed">
            Innovating the future of athletes. Explore Nike high-performance footwear and apparel engineered for speed, power, and comfort.
          </p>
        </div>
      ...
      <footer className="bg-surface-container-lowest border-t border-white/5">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-12 w-full max-w-[1440px] mx-auto px-6 lg:px-12 py-32">
