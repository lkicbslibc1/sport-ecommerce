...
export default function Football({ onViewChange, user, setUser, cart, addToCart }) {
  ...
  // Filter products by Running category - now using products from context
  const categoryProducts = useMemo(() => {
    return products.filter(p => p.sportType === 'Running');
  }, [products]);
    ...
    >
      ...
      <main className="max-w-max-width mx-auto px-6 lg:px-12 py-base pt-28">
        <nav className="py-8 flex items-center gap-2 font-label-sm text-on-surface-variant uppercase tracking-widest">
          ...
          <span className="text-primary font-black">Running</span>
        </nav>

        <div className="mb-10">
          <h1 className="font-display-lg text-7xl md:text-8xl font-black italic tracking-tighter uppercase text-on-background">Running</h1>
          <p className="text-on-surface-variant max-w-xl mt-4 font-body-lg font-light leading-relaxed">
            Engineered for precision. Built for control. Explore our latest Running performance apparel.
          </p>
        </div>
